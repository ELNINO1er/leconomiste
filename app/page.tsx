...
import {brevesVersStories,carteVersArticle,IMAGE_PAR_DEFAUT,slugifier} from "../lib/adapt";
...
export default async function Home(){
 ...
 // La page s'enrichit avec le fonds éditorial. Tant que le journal compte peu
 // d'articles, mieux vaut une édition courte que le même titre dans quatre blocs.
 const slider=une.length?une.slice(0,4):derniers.slice(0,1);
 const utilises=new Set(slider.map(a=>a.slug));
 const colonne=derniers.filter(a=>!utilises.has(a.slug)).slice(0,3);
 colonne.forEach(a=>utilises.add(a.slug));

 const economie=derniers.filter(a=>slugifier(a.category)==="economie"&&!utilises.has(a.slug));
 const ecoUne=economie.length>=3?economie.slice(0,5):[];
 ecoUne.forEach(a=>utilises.add(a.slug));

 const candidatsDecideurs=derniers.filter(a=>!utilises.has(a.slug));
 const decideurs=candidatsDecideurs.length>=3?candidatsDecideurs.slice(0,3):[];
 decideurs.forEach(a=>utilises.add(a.slug));

 const plusLusUniques=lesPlusLus.filter(a=>!utilises.has(a.slug));
 const plusLus=plusLusUniques.length>=4?plusLusUniques.slice(0,6):[];
 ...
 {plusLus.length>0&&<section className="daily-section shell"><div className="daily-section-title"><div><span>LES PLUS LUS</span><h2>Ce que lisent les décideurs</h2></div><Link href="/explorer">Toutes les publications →</Link></div><div className="most-read">{plusLus.map((a,i)=><Link href={`/articles/${a.slug}`} key={a.slug}><span>{String(i+1).padStart(2,"0")}</span><div><small>{a.rubrique.name}</small><h3>{a.titre}</h3></div><b>{nombre.format(a.vues)} lectures</b></Link>)}</div></section>}
